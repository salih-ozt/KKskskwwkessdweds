...
// ════════════════════════════════════════════════════════════════════
// 🛡️ ŞİKAYET YETKİLİSİ PANELİ — /sikayet/
// ════════════════════════════════════════════════════════════════════

const SIKAYET_COOKIE = 'sk_session';

function sikayetHtml(title, body) {
    return `<!DOCTYPE html>
<html lang="tr">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>${title} — Agro Sosyal Şikayet Paneli</title>
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    :root{
      --bg:#060d18;--surface:#0d1526;--surface2:#111d33;
      --border:rgba(255,255,255,0.07);--border2:rgba(255,255,255,0.04);
      --text:#e2e8f0;--muted:#475569;--muted2:#334155;
    ...
    .badge-email{background:rgba(59,130,246,.1);border:1px solid rgba(59,130,246,.2);color:#93c5fd;padding:5px 12px;border-radius:9999px;font-size:12px}
    .btn-logout{background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.2);color:#fca5a5;padding:6px 14px;border-radius:8px;font-size:12px;cursor:pointer;font-family:inherit}
    ...
    .filter-btn{padding:7px 16px;border-radius:9999px;font-size:12px;font-weight:600;cursor:pointer;border:1px solid var(--border);background:var(--surface);color:var(--muted);font-family:inherit;transition:all .15s}
    ...
    .filter-btn.active-green{background:var(--green);border-color:var(--green);color:#fff}
    ...
    th{padding:12px 16px;text-align:left;font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.6px;border-bottom:1px solid var(--border2);background:var(--surface2)}
    td{padding:14px 16px;font-size:13px;border-bottom:1px solid var(--border2);vertical-align:middle}
    tr:last-child td{border-bottom:none}
    tr:hover td{background:rgba(255,255,255,.015)}
    .badge{display:inline-flex;align-items:center;gap:4px;padding:3px 10px;border-radius:9999px;font-size:11px;font-weight:700}
    .badge-pending{background:rgba(245,158,11,.12);color:#fbbf24;border:1px solid rgba(245,158,11,.2)}
    .badge-reviewed{background:rgba(34,197,94,.12);color:#4ade80;border:1px solid rgba(34,197,94,.2)}
    .badge-dismissed{background:rgba(71,85,105,.2);color:#94a3b8;border:1px solid rgba(71,85,105,.3)}
    .reason-tag{display:inline-block;padding:2px 8px;border-radius:6px;font-size:11px;font-weight:600;background:rgba(239,68,68,.1);color:#fca5a5;border:1px solid rgba(239,68,68,.15)}
    .post-link{display:inline-flex;align-items:center;gap:5px;color:var(--primary);font-size:12px;background:rgba(59,130,246,.08);border:1px solid rgba(59,130,246,.15);padding:4px 10px;border-radius:7px}
    .post-link:hover{background:rgba(59,130,246,.15);text-decoration:none}
    .action-btn{padding:5px 12px;border-radius:7px;font-size:11px;font-weight:600;cursor:pointer;border:none;font-family:inherit;transition:all .15s}
    .btn-resolve{background:rgba(34,197,94,.1);color:#4ade80;border:1px solid rgba(34,197,94,.2)}
    .btn-resolve:hover{background:rgba(34,197,94,.25)}
    .btn-dismiss{background:rgba(71,85,105,.15);color:#94a3b8;border:1px solid rgba(71,85,105,.2)}
    .btn-dismiss:hover{background:rgba(71,85,105,.3)}
    .empty{text-align:center;padding:60px 20px;color:var(--muted)}
    .empty-icon{font-size:48px;margin-bottom:12px}
    .user-cell{display:flex;flex-direction:column;gap:2px}
    .user-name{font-weight:600;font-size:13px}
    .user-id{font-size:10px;color:var(--muted2);font-family:monospace}
    .date-cell{font-size:12px;color:var(--muted)}
    .desc-cell{font-size:12px;color:var(--muted);max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    /* Giriş sayfası */
    .login-wrap{min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;background:var(--bg);background-image:radial-gradient(ellipse 60% 40% at 50% 0%,rgba(59,130,246,.07),transparent)}
    .login-card{background:var(--surface);border:1px solid var(--border);border-radius:28px;padding:44px 40px;width:100%;max-width:420px;position:relative;overflow:hidden}
    .login-card::before{content:'';position:absolute;inset-x:0;top:0;height:1px;background:linear-gradient(90deg,transparent,rgba(59,130,246,.5),transparent)}
    ...
    .info-box{background:rgba(59,130,246,.07);border:1px solid rgba(59,130,246,.15);border-radius:12px;padding:14px 16px;margin-bottom:24px;font-size:13px;color:#93c5fd;line-height:1.6}
    .field{margin-bottom:16px}
    .field label{display:block;font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.7px;margin-bottom:7px}
    .field input{width:100%;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:12px;color:var(--text);font-size:14px;padding:13px 16px;outline:none;font-family:inherit;transition:border-color .2s,box-shadow .2s}
    .field input:focus{border-color:var(--primary);box-shadow:0 0 0 3px rgba(59,130,246,.12)}
    .field input::placeholder{color:var(--muted2)}
    .login-btn{width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#1d4ed8,#3b82f6);color:#fff;font-size:14px;font-weight:700;border:none;cursor:pointer;font-family:inherit;transition:opacity .2s,transform .15s;box-shadow:0 8px 24px rgba(59,130,246,.2)}
    .login-btn:hover{opacity:.9;transform:translateY(-1px)}
    .login-btn:active{transform:translateY(0)}
    .error-msg{background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);border-radius:10px;padding:10px 14px;color:#fca5a5;font-size:13px;margin-bottom:16px}
...
// ── GİRİŞ SAYFASI ──────────────────────────────────────────────────────────
app.get(['/sikayet', '/sikayet/'], async (req, res) => {
    const token = req.cookies?.[SIKAYET_COOKIE];
    if (token) {
        try {
            jwt.verify(token, JWT_SECRET, { algorithms: ['HS256'] });
            return res.redirect('/sikayet/panel');
        } catch {}
    }

    const err = req.query.hata;
    const errHtml = err === '1'
        ? `<div class="error-msg">❌ Bu e-posta ile onaylı bir şikayet yetkilisi kaydı bulunamadı.</div>`
        : err === '2'
        ? `<div class="error-msg">❌ Sunucu hatası. Lütfen tekrar dene.</div>`
        : '';

    res.type('html').send(sikayetHtml('Giriş', `
    <div class="login-wrap">
      <div class="login-card">
        <div class="login-logo">
          <div class="logo-dot"></div>
          <div>
            <div class="logo-title">Agro Sosyal</div>
            <div class="logo-sub" style="font-size:10px;color:#475569">Şikayet Yetkilisi Paneli</div>
          </div>
        </div>

        <div class="login-title">🛡️ Panele Giriş</div>
        <div class="login-sub">Bu panele yalnızca Agro Sosyal tarafından onaylanmış şikayet yetkilileri erişebilir.</div>

        <div class="info-box">
          📧 Giriş yapmak için <strong>başvuru formunda kullandığınız e-posta adresinizi</strong> giriniz. Başvurunuzun onaylanmış olması gerekmektedir.
        </div>

        ${errHtml}

        <form method="POST" action="/sikayet/login">
          <div class="field">
            <label>E-posta Adresi</label>
            <input type="email" name="email" placeholder="ornek@gmail.com" required autofocus>
          </div>
...
// ── GİRİŞ İŞLEMİ ───────────────────────────────────────────────────────────
app.post('/sikayet/login', async (req, res) => {
    try {
        ...
        const officer = await dbGet(
            `SELECT * FROM partnership_applications
             WHERE LOWER(email) = $1
               AND status = 'approved'
               AND LOWER("workField") LIKE '%şikayet%'
             LIMIT 1`,
            [email]
        );
        ...
        res.cookie(SIKAYET_COOKIE, token, {
            httpOnly: true,
            secure  : isSecure,
            sameSite: 'strict',
        ...
        console.error('[Şikayet Login]', err);
...
function requireSikayetAuth(req, res, next) {
    ...
    try {
        req.sikayetUser = jwt.verify(token, JWT_SECRET, { algorithms: ['HS256'] });
        next();
    } catch {
        res.clearCookie(SIKAYET_COOKIE);
        res.redirect('/sikayet/');
...
app.get('/sikayet/panel', requireSikayetAuth, async (req, res) => {
    try {
        const statusFilter = req.query.durum || 'all';

        let whereClause = '';
        if (statusFilter === 'pending')   whereClause = `WHERE r.status = 'pending'`;
        if (statusFilter === 'reviewed')  whereClause = `WHERE r.status = 'reviewed'`;
        if (statusFilter === 'dismissed') whereClause = `WHERE r.status = 'dismissed'`;
        ...
        const reports = await dbAll(`
            SELECT
                ...
                p.username   AS "postOwner",
                p.media      AS "postMedia",
                p."mediaType"
            FROM reports r
        ...
        const BASE = (process.env.APP_URL || 'https://sehitumitkestitarimmtal.com').replace(/\/$/, '');

        function statusBadge(s) {
            if (s === 'pending')   return '<span class="badge badge-pending">⏳ Bekliyor</span>';
            if (s === 'reviewed')  return '<span class="badge badge-reviewed">✅ İncelendi</span>';
            if (s === 'dismissed') return '<span class="badge badge-dismissed">❌ Reddedildi</span>';
        ...
        function reasonLabel(r) {
            ...
            return `<span class="reason-tag">${map[r] || r || '—'}</span>`;
        }
        ...
        const rows = reports.length === 0 ? `
            <tr><td colspan="7">
              <div class="empty">
                ...
                <div>Bu filtrede hiç şikayet yok.</div>
              </div>
            </td></tr>` :
            reports.map(r => {
                const postUrl = r.postId ? `${BASE}/post/${r.postId}` : null;
                const postCell = postUrl
                    ? `<a class="post-link" href="${postUrl}" target="_blank">🔗 Gönderiye Git</a>
                       ${r.postContent ? `<div class="desc-cell" style="margin-top:4px" title="${escapeHtml(r.postContent || '')}">${escapeHtml((r.postContent||'').slice(0,60))}${(r.postContent||'').length>60?'…':''}</div>` : ''}`
                    : '<span style="color:#334155;font-size:12px">Post silinmiş</span>';

                const actions = r.status === 'pending' ? `
                    <div style="display:flex;gap:6px">
                      <form method="POST" action="/sikayet/action/${r.id}/reviewed" style="display:inline">
                        <button class="action-btn btn-resolve" type="submit">✅ İncelendi</button>
                      </form>
                      <form method="POST" action="/sikayet/action/${r.id}/dismissed" style="display:inline">
                        <button class="action-btn btn-dismiss" type="submit">❌ Reddet</button>
                      </form>
                ...
                return `<tr>
                    <td>
                      <div class="user-cell">
                        <div class="user-name">${escapeHtml(r.reporterName || '—')}</div>
                        <div class="user-id">${escapeHtml(r.reporterEmail || '')}</div>
                      </div>
                    </td>
                    <td>${reasonLabel(r.reason)}</td>
                    <td>${postCell}</td>
                    <td class="desc-cell" title="${escapeHtml(r.description||'')}">
                      ${r.description ? escapeHtml(r.description.slice(0,80)) + (r.description.length>80?'…':'') : '<span style="color:#334155">—</span>'}
                    </td>
                    <td><div class="date-cell">${fmtDate(r.createdAt)}</div></td>
                    <td>${actions}</td>
                  </tr>`;
            }).join('');

        const filterBtns = [
            { val: 'all',       label: `Tümü (${stats.total || 0})` },
            { val: 'pending',   label: `⏳ Bekliyor (${stats.pending || 0})` },
            { val: 'reviewed',  label: `✅ İncelendi (${stats.reviewed || 0})` },
            { val: 'dismissed', label: `❌ Reddedildi (${stats.dismissed || 0})` },
        ].map(f => {
            const active = statusFilter === f.val;
            const cls = active
                ? (f.val==='pending'?'active-red':f.val==='reviewed'?'active-green':'active')
                : '';
            return `<a href="/sikayet/panel?durum=${f.val}" class="filter-btn ${cls}">${f.label}</a>`;
        }).join('');

        res.type('html').send(sikayetHtml('Panel', `
        <div class="topbar">
          <div class="topbar-left">
            <div class="logo-dot"></div>
            <div>
              <div class="logo-title">Şikayet Yetkilisi Paneli</div>
              <div class="logo-sub">Agro Sosyal</div>
            </div>
          </div>
          <div class="topbar-right">
            <span class="badge-email">👤 ${escapeHtml(req.sikayetUser.fullName || req.sikayetUser.email)}</span>
            <form method="POST" action="/sikayet/logout" style="display:inline">
              <button class="btn-logout" type="submit">Çıkış Yap</button>
            </form>
          </div>
        </div>
        ...
        <div class="container">
          <div class="stats-row">
            <div class="stat-card">
              <div class="stat-num" style="color:#f1f5f9">${stats.total || 0}</div>
              <div class="stat-label">Toplam Şikayet</div>
            </div>
            <div class="stat-card">
              <div class="stat-num" style="color:#fbbf24">${stats.pending || 0}</div>
              <div class="stat-label">⏳ Bekleyen</div>
            </div>
            <div class="stat-card">
              <div class="stat-num" style="color:#4ade80">${stats.reviewed || 0}</div>
              <div class="stat-label">✅ İncelenen</div>
            </div>
            <div class="stat-card">
              <div class="stat-num" style="color:#94a3b8">${stats.dismissed || 0}</div>
          ...
          <div class="table-wrap">
            <div class="table-head">
              <span class="table-head-title">🚨 Şikayet Listesi</span>
              <span style="font-size:12px;color:#475569">${reports.length} kayıt gösteriliyor</span>
            </div>
            <table>
              <thead>
                <tr>
        ...
        res.status(500).type('html').send(sikayetHtml('Hata', `<div style="text-align:center;padding:60px;color:#ef4444"><h2>Sunucu hatası oluştu.</h2><p>${err.message}</p></div>`));
    }
});

// ── DURUM GÜNCELLEME ────────────────────────────────────────────────────────
app.post('/sikayet/action/:id/:newStatus', requireSikayetAuth, async (req, res) => {
    try {
        ...
        await dbRun(
            `UPDATE reports SET status=$1, "reviewedAt"=NOW(), "reviewedBy"=$2 WHERE id=$3`,
            [newStatus, req.sikayetUser.email, id]
        );
