...
app.use(helmet({
    contentSecurityPolicy: {
        directives: {
            defaultSrc:     ["'self'"],
            // ✅ INLINE SCRIPT'LER İÇİN: 'unsafe-inline' eklendi
            scriptSrc:      ["'self'", "'unsafe-inline'", "https://cdn.tailwindcss.com", 
                             "https://unpkg.com", "https://cdnjs.cloudflare.com", 
                             "https://cdn.socket.io", "'unsafe-eval'"],
            styleSrc:       ["'self'", "'unsafe-inline'", "https://cdnjs.cloudflare.com",
                             "https://fonts.googleapis.com"],
            // ✅ TÜM HTTPS GÖRSELLER İÇİN: '*' wildcard eklendi
            imgSrc:         ["'self'", "data:", "blob:", "https:", "*"],
            connectSrc:     ["'self'", "wss:", "https:", "ws:"],
            fontSrc:        ["'self'", "https://cdnjs.cloudflare.com", 
                             "https://fonts.gstatic.com", "data:"],
            // ✅ KENDİ DOMAIN'INDEN IFRAME'LERE İZİN
            frameSrc:       ["'self'"],
            objectSrc:      ["'none'"],
            baseUri:        ["'self'"],
            formAction:     ["'self'"],
    ...
    // ✅ DÜZELTME: same-site -> cross-origin (harici kaynakların yüklenmesine izin)
...
// HTML'den çağrılan: axios.post(`/api/posts/${post.id}/view`)
// Feed'de veya profile'da post göründüğünde çağrılır
app.post('/api/posts/:id/view', authenticateToken, async (req, res) => {
    try {
        ...
        const post = await db.get('SELECT id, views FROM posts WHERE id = ? AND isActive = 1', id);
        if (!post) return res.status(404).json({ error: 'Post bulunamadı' });

        // Kullanıcı bazlı benzersiz görüntüleme (günde 1 kez)
        await incrementPostView(id, req.user.id, req.ip);

        // Güncel views sayısını dön
